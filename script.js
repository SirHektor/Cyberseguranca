...
function exibirAlerta() {
    alert("Por favor, registre-se");
}
function verificarRegistro() {
    // Obtém a força da senha
    var forcaSenha = verificarPoder(entradaSenha.value);

    // Define um limite de força para considerar o registro
    var limiteForca = 50; // Você pode ajustar conforme necessário

    // Se a força da senha atender ao limite
    if (forcaSenha >= limiteForca) {
        // Oculta o contêiner de registro
        document.querySelector('.container').classList.add('oculto');

        // Exibe a mensagem de registro
        document.getElementById('mensagemRegistro').classList.remove('oculto');
    } else {
        // Se a força da senha não atender ao limite, exibe um alerta
